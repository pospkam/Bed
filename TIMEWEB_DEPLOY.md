# 🚀 Деплой KAMHUB на Timeweb Cloud

Пошаговая инструкция развертывания Next.js проекта на российском VPS хостинге Timeweb Cloud.

---

## 📋 Что вам понадобится

- [ ] Аккаунт на [Timeweb Cloud](https://timeweb.cloud/)
- [ ] Баланс от 300₽/мес (тариф Cloud 1)
- [ ] Telegram Bot Token (от @BotFather)
- [ ] Chat ID для уведомлений
- [ ] Домен (опционально, можно работать по IP)

---

## ⚡ БЫСТРЫЙ СТАРТ (автоматический деплой)

### Вариант 1: Один скрипт для всего

```bash
# 1. Создайте сервер в Timeweb Cloud (Ubuntu 22.04)
# 2. Подключитесь по SSH:
ssh root@ваш-ip-адрес

# 3. Запустите автоматическую установку:
curl -fsSL https://raw.githubusercontent.com/pospkam/HabKam/main/scripts/timeweb-install.sh | bash
```

**Скрипт автоматически:**
- ✅ Установит Node.js 18
- ✅ Установит PM2, Nginx, Git
- ✅ Клонирует репозиторий
- ✅ Соберёт проект
- ✅ Настроит автозапуск
- ✅ Настроит Nginx
- ✅ Запустит сайт

**Время: 5-10 минут**

---

## 🛠️ ПОШАГОВАЯ УСТАНОВКА (ручная)

### Шаг 1: Создание сервера

**В панели Timeweb Cloud:**

1. Перейдите: **Серверы** → **Создать сервер**

2. **Конфигурация:**
   ```
   Операционная система: Ubuntu 22.04 LTS
   
   Тариф Cloud 1 (минимум):
   - 1 vCPU
   - 1 GB RAM  
   - 10 GB SSD
   - Цена: ~300₽/мес
   
   Тариф Cloud 2 (рекомендую):
   - 2 vCPU
   - 2 GB RAM
   - 20 GB SSD
   - Цена: ~600₽/мес
   
   Регион: Москва или Санкт-Петербург
   ```

3. **Доступ:**
   - SSH ключ (добавьте свой публичный ключ)
   - Или используйте пароль (придёт на email)

4. Нажмите **"Создать сервер"**

5. Дождитесь готовности (2-3 минуты)

---

### Шаг 2: Подключение по SSH

**Получите IP адрес в панели Timeweb**

```bash
# Подключитесь к серверу
ssh root@123.456.78.90

# При первом подключении:
# Are you sure you want to continue connecting? → yes

# Вы должны быть в консоли сервера:
root@server:~#
```

---

### Шаг 3: Установка зависимостей

```bash
# Обновите систему
apt update && apt upgrade -y

# Установите необходимые пакеты
apt install -y curl wget git build-essential

# Установите Node.js 18 (официальный репозиторий)
curl -fsSL https://deb.nodesource.com/setup_18.x | bash -
apt install -y nodejs

# Проверьте версии
node --version    # v18.x.x
npm --version     # 9.x.x

# Установите PM2 (менеджер процессов Node.js)
npm install -g pm2

# Установите Nginx (веб-сервер)
apt install -y nginx

# Включите автозапуск Nginx
systemctl enable nginx
systemctl start nginx
```

---

### Шаг 4: Клонирование проекта

```bash
# Создайте директорию для проектов
mkdir -p /var/www
cd /var/www

# Клонируйте репозиторий
git clone https://github.com/pospkam/HabKam.git
cd HabKam

# Проверьте что код загрузился
ls -la
# Должны увидеть: app/, components/, lib/, package.json, etc.
```

---

### Шаг 5: Настройка переменных окружения

```bash
# Создайте .env.local файл
nano .env.local
```

**Вставьте (замените на свои значения):**

```env
# Telegram Bot
TELEGRAM_BOT_TOKEN=123456789:ABCdefGHIjklMNOpqrsTUVwxyz
TELEGRAM_CHAT_ID=123456789

# Application URL (замените на свой IP или домен)
NEXT_PUBLIC_SITE_URL=http://123.456.78.90
```

**Как получить токены:**

1. **Bot Token:**
   - Telegram → найдите @BotFather
   - Отправьте: `/newbot`
   - Следуйте инструкциям
   - Скопируйте токен

2. **Chat ID:**
   - Отправьте боту любое сообщение
   - Откройте: `https://api.telegram.org/bot<YOUR_TOKEN>/getUpdates`
   - Найдите `"chat":{"id": NUMBER}`
   - Скопируйте число

**Сохраните:** `Ctrl+X` → `Y` → `Enter`

---

### Шаг 6: Сборка проекта

```bash
# Установите зависимости
npm install --production

# Соберите production build
npm run build

# Это займёт 1-3 минуты
# В конце должно быть:
# ✓ Compiled successfully

# Проверьте что сборка создалась
ls -la .next/
```

---

### Шаг 7: Запуск с PM2

```bash
# Запустите приложение
pm2 start npm --name "kamhub" -- start

# Проверьте статус
pm2 status

# Должно быть:
┌─────┬─────────┬─────────┬─────────┬─────────┬──────────┐
│ id  │ name    │ mode    │ status  │ cpu     │ memory   │
├─────┼─────────┼─────────┼─────────┼─────────┼──────────┤
│ 0   │ kamhub  │ fork    │ online  │ 0%      │ 50.0mb   │
└─────┴─────────┴─────────┴─────────┴─────────┴──────────┘

# Настройте автозапуск при перезагрузке сервера
pm2 startup systemd
pm2 save

# Проверьте логи (опционально)
pm2 logs kamhub --lines 20
```

**PM2 команды:**
```bash
pm2 status           # Статус всех процессов
pm2 restart kamhub   # Перезапуск
pm2 stop kamhub      # Остановка
pm2 logs kamhub      # Логи в реальном времени
pm2 monit            # Интерактивный мониторинг
```

---

### Шаг 8: Настройка Nginx

```bash
# Создайте конфигурацию для сайта
nano /etc/nginx/sites-available/kamhub
```

**Вставьте конфигурацию:**

```nginx
server {
    listen 80;
    listen [::]:80;
    
    # Замените на свой домен или IP
    server_name kamhub.ru www.kamhub.ru;
    # Или если нет домена:
    # server_name 123.456.78.90;

    # Логи
    access_log /var/log/nginx/kamhub-access.log;
    error_log /var/log/nginx/kamhub-error.log;

    # Next.js приложение
    location / {
        proxy_pass http://localhost:3000;
        proxy_http_version 1.1;
        
        # WebSocket поддержка
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        
        # Заголовки
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        
        proxy_cache_bypass $http_upgrade;
        
        # Таймауты
        proxy_connect_timeout 60s;
        proxy_send_timeout 60s;
        proxy_read_timeout 60s;
    }

    # Статика Next.js (кеширование)
    location /_next/static {
        alias /var/www/HabKam/.next/static;
        expires 365d;
        access_log off;
        add_header Cache-Control "public, immutable";
    }

    # Изображения
    location /images {
        alias /var/www/HabKam/public/images;
        expires 30d;
        access_log off;
        add_header Cache-Control "public";
    }

    # Безопасность
    add_header X-Frame-Options "SAMEORIGIN" always;
    add_header X-Content-Type-Options "nosniff" always;
    add_header X-XSS-Protection "1; mode=block" always;

    # Максимальный размер загружаемых файлов
    client_max_body_size 10M;
}
```

**Сохраните:** `Ctrl+X` → `Y` → `Enter`

**Активируйте конфигурацию:**

```bash
# Создайте символическую ссылку
ln -s /etc/nginx/sites-available/kamhub /etc/nginx/sites-enabled/

# Удалите дефолтный конфиг (опционально)
rm /etc/nginx/sites-enabled/default

# Проверьте конфигурацию на ошибки
nginx -t

# Должно быть:
# nginx: the configuration file /etc/nginx/nginx.conf syntax is ok
# nginx: configuration file /etc/nginx/nginx.conf test is successful

# Перезагрузите Nginx
systemctl reload nginx
```

---

### Шаг 9: Настройка Firewall (опционально)

```bash
# Установите UFW
apt install -y ufw

# Разрешите SSH (важно!)
ufw allow 22/tcp

# Разрешите HTTP и HTTPS
ufw allow 80/tcp
ufw allow 443/tcp

# Включите firewall
ufw --force enable

# Проверьте статус
ufw status
```

---

### Шаг 10: Настройка домена (опционально)

#### В панели Timeweb Cloud:

1. **DNS** → **Добавить домен**
2. Введите ваш домен: `kamhub.ru`
3. Создайте записи:

**A-запись (основной домен):**
```
Тип: A
Имя: @
Значение: [IP вашего сервера]
TTL: 3600
```

**CNAME-запись (www):**
```
Тип: CNAME
Имя: www
Значение: kamhub.ru
TTL: 3600
```

#### У регистратора домена:

Измените NS-серверы на:
```
ns1.timeweb.ru
ns2.timeweb.ru
ns3.timeweb.org
ns4.timeweb.org
```

**Ожидание:** 2-24 часа (обычно 2-4 часа)

---

### Шаг 11: SSL сертификат (HTTPS)

```bash
# Установите Certbot
apt install -y certbot python3-certbot-nginx

# Получите SSL сертификат (замените домен на свой)
certbot --nginx -d kamhub.ru -d www.kamhub.ru

# Certbot спросит:
# 1. Email для уведомлений → введите свой email
# 2. Согласие с Terms of Service → Y
# 3. Получать рассылку → N (по желанию)
# 4. Redirect HTTP to HTTPS? → 2 (Yes, redirect)

# Проверьте автопродление
certbot renew --dry-run

# Настройте крон для автопродления
echo "0 12 * * * /usr/bin/certbot renew --quiet" | crontab -
```

**После этого сайт будет доступен по HTTPS!**

---

## ✅ ПРОВЕРКА РАБОТЫ

### 1. Проверьте доступность

```bash
# По IP
curl http://ваш-ip-адрес

# По домену (если настроен)
curl http://kamhub.ru
curl https://kamhub.ru
```

### 2. Откройте в браузере

```
http://ваш-ip-адрес
или
https://kamhub.ru
```

### 3. Тест бронирования

1. Перейдите: **Туры**
2. Выберите тур
3. Нажмите **Забронировать**
4. Заполните форму
5. Отправьте
6. **Проверьте Telegram** - должно прийти уведомление!

---

## 🔄 ОБНОВЛЕНИЕ КОДА

```bash
# Подключитесь к серверу
ssh root@ваш-ip

# Перейдите в директорию проекта
cd /var/www/HabKam

# Остановите приложение
pm2 stop kamhub

# Получите последние изменения
git pull origin main

# Обновите зависимости (если изменились)
npm install --production

# Пересоберите проект
npm run build

# Запустите приложение
pm2 restart kamhub

# Проверьте логи
pm2 logs kamhub --lines 50
```

**Автоматизированный скрипт обновления:**

```bash
# Создайте скрипт
nano /root/update-kamhub.sh
```

```bash
#!/bin/bash
cd /var/www/HabKam
pm2 stop kamhub
git pull origin main
npm install --production
npm run build
pm2 restart kamhub
echo "✅ Обновление завершено!"
```

```bash
# Сделайте исполняемым
chmod +x /root/update-kamhub.sh

# Используйте так:
/root/update-kamhub.sh
```

---

## 📊 МОНИТОРИНГ

### PM2 мониторинг

```bash
# Статус процессов
pm2 status

# Детальная информация
pm2 show kamhub

# Логи в реальном времени
pm2 logs kamhub

# Интерактивный монитор
pm2 monit

# Очистить логи
pm2 flush
```

### Системный мониторинг

```bash
# Использование CPU/RAM
htop

# Место на диске
df -h

# Процессы Node.js
ps aux | grep node

# Порты
netstat -tulpn | grep :3000
```

### Логи Nginx

```bash
# Логи доступа
tail -f /var/log/nginx/kamhub-access.log

# Логи ошибок
tail -f /var/log/nginx/kamhub-error.log

# Последние 100 строк
tail -100 /var/log/nginx/kamhub-error.log
```

---

## ⚙️ ОПТИМИЗАЦИЯ

### Добавить SWAP (если RAM < 2GB)

```bash
# Создать файл подкачки 2GB
fallocate -l 2G /swapfile
chmod 600 /swapfile
mkswap /swapfile
swapon /swapfile

# Сделать постоянным
echo '/swapfile none swap sw 0 0' >> /etc/fstab

# Проверить
free -h
```

### Настроить кеширование Nginx

```bash
# Добавьте в /etc/nginx/nginx.conf в блок http:
nano /etc/nginx/nginx.conf
```

```nginx
# Кеш для статики
proxy_cache_path /var/cache/nginx levels=1:2 keys_zone=nextjs_cache:10m max_size=100m inactive=60m use_temp_path=off;
```

### Ротация логов

```bash
# Настроить logrotate
nano /etc/logrotate.d/kamhub
```

```
/var/log/nginx/kamhub-*.log {
    daily
    rotate 14
    compress
    delaycompress
    notifempty
    create 0640 www-data adm
    sharedscripts
    postrotate
        systemctl reload nginx
    endscript
}
```

---

## ❗ РЕШЕНИЕ ПРОБЛЕМ

### Сайт не открывается (502 Bad Gateway)

```bash
# Проверьте что Next.js запущен
pm2 status

# Если offline - запустите
pm2 restart kamhub

# Проверьте логи
pm2 logs kamhub --lines 100

# Проверьте порт 3000
netstat -tulpn | grep :3000

# Логи Nginx
tail -50 /var/log/nginx/kamhub-error.log
```

### Приложение падает / крашится

```bash
# Увеличьте лимит памяти
pm2 delete kamhub
pm2 start npm --name "kamhub" --max-memory-restart 500M -- start

# Добавьте SWAP (см. раздел Оптимизация)

# Проверьте логи на ошибки
pm2 logs kamhub --err
```

### Медленно работает

```bash
# Проверьте ресурсы
htop

# CPU > 90% → обновите тариф
# RAM > 90% → добавьте SWAP или обновите тариф

# Проверьте I/O диска
iostat -x 1
```

### Telegram не отправляет

```bash
# Проверьте переменные окружения
cat /var/www/HabKam/.env.local

# Проверьте токен вручную
curl "https://api.telegram.org/bot<YOUR_TOKEN>/getMe"

# Должен вернуть информацию о боте

# Проверьте логи приложения
pm2 logs kamhub | grep telegram
```

### Домен не открывается

```bash
# Проверьте DNS
nslookup kamhub.ru

# Проверьте конфиг Nginx
nginx -t

# Проверьте что домен указан в server_name
grep server_name /etc/nginx/sites-available/kamhub
```

---

## 💰 СТОИМОСТЬ

**Timeweb Cloud тарифы:**

| Тариф | vCPU | RAM | SSD | Цена/мес |
|-------|------|-----|-----|----------|
| Cloud 1 | 1 | 1 GB | 10 GB | ~300₽ |
| Cloud 2 | 2 | 2 GB | 20 GB | ~600₽ |
| Cloud 3 | 3 | 4 GB | 40 GB | ~1200₽ |

**Рекомендация:**
- Тестирование: Cloud 1
- Продакшн: Cloud 2 или выше

**Дополнительно:**
- Домен: от 150₽/год
- SSL сертификат: бесплатно (Let's Encrypt)
- Бэкапы: опционально

---

## 📚 ПОЛЕЗНЫЕ ССЫЛКИ

- 📘 [Документация Timeweb Cloud](https://timeweb.cloud/help)
- 📗 [Next.js Deployment](https://nextjs.org/docs/deployment)
- 📕 [PM2 Documentation](https://pm2.keymetrics.io/)
- 📙 [Nginx Configuration](https://nginx.org/ru/docs/)

---

## 🎯 ЧЕКЛИСТ

- [ ] Сервер создан в Timeweb Cloud
- [ ] Подключение по SSH работает
- [ ] Node.js 18+ установлен
- [ ] PM2 установлен
- [ ] Nginx установлен
- [ ] Код клонирован в /var/www/HabKam
- [ ] .env.local создан с правильными токенами
- [ ] npm run build выполнен успешно
- [ ] PM2 запущен (pm2 status → online)
- [ ] Nginx настроен
- [ ] Сайт открывается по IP
- [ ] Домен настроен (опционально)
- [ ] SSL настроен (опционально)
- [ ] Тест бронирования прошёл успешно
- [ ] Telegram уведомление получено

---

## 🚀 ГОТОВО!

**Ваш сайт работает на Timeweb Cloud!**

**URL:** `http://ваш-ip` или `https://ваш-домен.ru`

**Поддержка:**
- 📧 Email: info@kamhub.ru
- 💬 Telegram: @kamhub_support
- 🐛 Issues: https://github.com/pospkam/HabKam/issues

**Успешного запуска! 🎣🏔️**
